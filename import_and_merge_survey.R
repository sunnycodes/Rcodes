...
#import csv in R
#loop though the files and store them in individual R data frames
#i<-1
#for (i in 1:23) {
  raw<-read.csv(file="/Users/sunnypiya/Desktop/renamed/raw")
#}
...
dflist<-list(raw1, raw2, raw3, raw4, raw5, raw6, raw7, raw8, raw9, raw10, raw11, raw12, raw13, raw14, raw15,
          raw16, raw17, raw18, raw19, raw20, raw21, raw22, raw23)

#remove the first row form all data frames in the dflist and assign a new df name
#k<-1
for (k in 1:23){
  assign(paste("raw",k,"new",sep=""), remove_row1(dflist[[k]]))
}

#bind all data frames into a single data frame
dflistnew<-list(raw1new, raw2new, raw3new, raw4new, raw5new, raw6new, raw7new, raw8new, raw9new, raw10new, raw11new, raw12new, raw13new, raw14new, raw15new,
             raw16new, raw17new, raw18new, raw19new, raw20new, raw21new, raw22new, raw23new)

#select only useful fields from the raw table 
#j<-0
for (j in 1:length(dflistnew)){
  assign(paste("raw",j,"trim", sep=""), data.frame(dflistnew[[j]]$email,
                                                   dflistnew[[j]]$RegR,
                                                   dflistnew[[j]]$SegR,
...
dflisttrim<-rbind(raw1trim, raw2trim, raw3trim,raw4trim,raw5trim,raw6trim,raw7trim,raw8trim,raw9trim,raw10trim,raw11trim,raw12trim,raw13trim,raw14trim,
                 raw15trim,raw16trim,raw17trim,raw18trim,raw19trim,raw20trim,raw21trim,raw22trim,raw23trim)
...
#output files from R
write.csv(dflisttrim, file = "/Users/sunnypiya/Desktop/scipt_test_new.csv")
