...
for (j in 1:length(dflistnew)){
             dflisttrim[[j]] <- data.frame(dflistnew[[j]]$V5,
                                                   dflistnew[[j]]$RegR,
                                                   dflistnew[[j]]$SegR,
...
#bind all rows for trimmed dataframes into a single dataframe
dflisttrim_rbind<-do.call("rbind", dflisttrim)
         
#assign new column names to the dataframe based on the TA database requirements
...
write.csv(dffinal, file = "~/Desktop/mac2.csv")
